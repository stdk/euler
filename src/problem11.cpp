...
int main() {
    ...
    for(size_t i=0;i<lines - ((adjacent_numbers - 1));i++) {
        for(size_t j=0;j<line_size;j++) {
            ...
            for(size_t k=0;k<adjacent_numbers;k++) {
                current_product *= numbers[(i+k)*line_size + j];
            }
    ...
    for(size_t i=0;i<lines - (adjacent_numbers - 1);i++) {
        for(size_t j=0;j<line_size - (adjacent_numbers - 1);j++) {
            ...
            size_t current_product_b = 1;
            for(size_t k=0;k<adjacent_numbers;k++) {
                current_product_a *= numbers[(i+k)*line_size + j + k];
                current_product_b *= numbers[(i+k)*line_size + j + (adjacent_numbers - 1) - k];
            }
            if(current_product_a > max) {
                max = current_product_a;
            }
